...
class CompileServiceServerSideImpl(
    ...
    suspend override fun processMessage(msg: AnyMessage, output: ByteWriteChannelWrapper) = when (msg) {
        is EndConnectionMessage -> Server.State.CLOSED
        is Message -> Server.State.WORKING
            .also { msg.process(this, output) }
    ...
    suspend override fun attachClient(client: Socket) {
        async {
            ...
            loop@ while (true) {
                when (processMessage(input.nextObject() as AnyMessage, output)) {
